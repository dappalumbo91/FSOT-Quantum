...
What stayed open (not a retune): Gset champions unmatched (G17 **13** edges, G22 **89**; G23 now 50 short). RSA-shaped **119-bit 8/8**; **64-bit 0/8** at locked ECM (ρ \(\sim 2^{32}\) wall). RSA-2048 not run. Multiprover **FSOT_QUANTUM_MULTIPROVER_OK** (37 obligations). Extra n=2000 modes + BFS as add-on refine moved G22 98→89 and G23 73→50. Classical ladder: `docs/CLASSICAL_RECORDS.md`. H2 Kolos derived 0.75% written; LiH FCI not invented (`docs/VQE_OBJECT.md`). Known-answer bank **44/44**. Vendor `BR_H_gg` field still stale. Exclusive \(B\to D\ell\nu\) is **0.15%**. SH0ES is Lean BH→WH at **1.00%**. \(\alpha_s(M_Z)\) is inside the vendor 0.9% band.
