...
What stayed open (not a retune): Gset champions unmatched after the <1% aspiration landed for all 11 (G17 **0.427%**, 13 edges; G22 **0.734%**, 98 edges). ECM **8/8** at 38-bit and **8/8** at 41-bit; RSA-2048 not run. Vendor `BR_H_gg` field still stale. Exclusive \(B\to D\ell\nu\) is **0.15%**. SH0ES is Lean BH→WH at **1.00%**. \(\alpha_s(M_Z)\) is inside the vendor 0.9% band.
