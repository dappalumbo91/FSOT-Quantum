...
| Shor / RSA | period and factor | modular + log-N (p−1/p+1/kN/ECM) + rho | far ρ **8/8**; log-N **8/8**; ECM through **80-bit 8/8**. 90-bit ECM exhausted at the same B. RSA-2048 not run. |
...
Current score is not a refusal. Factoring is far ρ **8/8**, log-N **8/8**, and ECM through **80-bit** today. A 90-bit far pair **exhausts** the same B/B2 (not retuned). RSA-2048 not run. Champion MaxCut is 13–98 edges short **today**. [`HIRED_JOBS.md`](HIRED_JOBS.md).
...
| Period / factor (Shor job) | far ρ **8/8** · log-N **8/8** · ECM through **80-bit 8/8** | 90-bit ECM exhausted at the same B. RSA-2048 not run. |
