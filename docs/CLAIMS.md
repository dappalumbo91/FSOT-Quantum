...
| Shor / RSA | period and factor | modular + log-N (p−1/p+1/kN/ECM) + rho | ECM **80-bit**; RSA-shaped **81 / 95 / 103 / 111 / 119-bit**. **64-bit 0/8** ECM wall. RSA-2048 not run. |
...
Current score is not a refusal. Factoring is far ρ **8/8**, log-N **8/8**, ECM through **80-bit**, and RSA-shaped through **119-bit 8/8** (60-bit ρ) today. **64-bit is 0/8** at the locked ECM B; ρ is \(\sim 2^{32}\) and was not finished. RSA-2048 not run. Champion MaxCut is 13–89 edges short **today**. [`HIRED_JOBS.md`](HIRED_JOBS.md).
...
| Period / factor (Shor job) | far ρ **8/8** · log-N **8/8** · ECM through **80-bit 8/8** · RSA-shaped **119-bit 8/8** | **64-bit 0/8** at locked ECM; ρ \(\sim 2^{32}\) wall. RSA-2048 not run. |
