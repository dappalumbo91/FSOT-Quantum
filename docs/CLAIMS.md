...
| Heights 16 RSA-shaped | **8/8** 64-bit × 64-bit (127-bit N) — CFRAC 5 + Brent ρ 3 | `python -m fsot_quantum heights16` | `HEIGHTS16.md` |
...
| Shor / RSA | period and factor | modular + log-N (p−1/p+1/kN/ECM) + rho + CFRAC | ECM **80-bit**; RSA-shaped **81 / 95 / 103 / 111 / 119-bit**. **64-bit 8/8** (CFRAC + Brent ρ). RSA-2048 not run. |
...
Current score is not a refusal. Factoring is far ρ **8/8**, log-N **8/8**, ECM through **80-bit**, RSA-shaped through **119-bit 8/8** (60-bit ρ), and **64-bit 8/8** (CFRAC + Brent ρ) today. RSA-2048 not run. Champion MaxCut is 13–89 edges short **today**. [`HIRED_JOBS.md`](HIRED_JOBS.md).
