...
| Heights 16 RSA-shaped | **0/8** 64-bit × 64-bit (127-bit N) — ECM locked B, ρ wall | `python -m fsot_quantum heights16` | `HEIGHTS16.md` |
...
| Shor / RSA | period and factor | modular + log-N (p−1/p+1/kN/ECM) + rho | ECM **80-bit**; RSA-shaped **81 / 95 / 103 / 111-bit**. **64-bit 0/8** ECM wall. RSA-2048 not run. |
...
Current score is not a refusal. Factoring is far ρ **8/8**, log-N **8/8**, ECM through **80-bit**, and RSA-shaped through **111-bit 8/8** today. **64-bit is 0/8** at the locked ECM B; ρ is \(\sim 2^{32}\) and was not finished. RSA-2048 not run. Champion MaxCut is 13–98 edges short **today**. [`HIRED_JOBS.md`](HIRED_JOBS.md).
...
| Gset champions | 13–98 edges short | Aspiration <1% landed **11/11**. Extra n=2000 modes moved G22 98→89 but displaced G23 73→85 — reverted. Do not advertise champion-matching. |
...
| Period / factor (Shor job) | far ρ **8/8** · log-N **8/8** · ECM through **80-bit 8/8** · RSA-shaped **111-bit 8/8** | **64-bit 0/8** at locked ECM; ρ \(\sim 2^{32}\) wall. RSA-2048 not run. |
