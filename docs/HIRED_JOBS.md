...
|---------------------|--------------|-----------|--------------|----------------|
| Shor | Factor \(N = pq\) | modular + p−1/p+1/kN + ECM + ρ | RSA-shaped **81 / 95 / 103 / 111-bit** | Vs QPU: ahead. Vs GNFS: climbing. [`CLASSICAL_RECORDS.md`](CLASSICAL_RECORDS.md). |
...
1. **Shor’s end-job** — RSA-shaped similar-bit primes through **111-bit**. Next classical rungs: 64-bit \(p\) (\(\sim 2^{32}\) ρ), then RSA-100 (330-bit). RSA-2048 still not run.
