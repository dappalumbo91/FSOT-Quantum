...
|---------------------|--------------|-----------|--------------|----------------|
| Shor | Factor \(N = pq\) | modular + p−1/p+1/kN + ECM + ρ | RSA-shaped **81 / 95 / 103 / 111 / 119-bit**; **64-bit 0/8** ECM | Vs QPU: ahead. Vs GNFS: 64-bit ρ wall. [`CLASSICAL_RECORDS.md`](CLASSICAL_RECORDS.md). |
...
1. **Shor’s end-job** — RSA-shaped similar-bit primes through **119-bit** (60-bit ρ **8/8**). **64-bit 0/8** at locked ECM; ρ \(\sim 2^{32}\) is the wall on this PC. Next named challenge: RSA-100 (330-bit). RSA-2048 still not run.
