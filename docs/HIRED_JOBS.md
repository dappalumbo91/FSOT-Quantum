...
|---------------------|--------------|-----------|--------------|----------------|
| Shor | Factor \(N = pq\) | modular + p−1/p+1/kN + ECM + ρ + CFRAC | RSA-shaped **81 / 95 / 103 / 111 / 119-bit**; **64-bit 5/8** CFRAC | Vs QPU: ahead. Vs GNFS: 3 of 8 64-bit still miss; RSA-100 next. [`CLASSICAL_RECORDS.md`](CLASSICAL_RECORDS.md). |
...
| QAOA / annealer | MaxCut / Ising | fold + KL + 2-opt + BFS / spectral | **11/11 Gset under 1%** of published champion | Aspiration met. **Champions unmatched** (G17 13 edges, G22 89, G1 39). |
...
1. **Shor’s end-job** — RSA-shaped similar-bit primes through **119-bit** (60-bit ρ **8/8**) and **64-bit 5/8** CFRAC at locked B. Three 64-bit pairs exhausted. Next named challenge: RSA-100 (330-bit). RSA-2048 still not run.
2. **QAOA’s end-job** — Gset MaxCut vs published champions. Family 11/11 under 1%. G17 **13 edges** and G22 **89 edges** are the leftovers. Not crawled.
