...
|---------------------|--------------|-----------|--------------|----------------|
| Shor | Factor \(N = pq\) | modular + p−1/p+1/kN + ECM + ρ | far ρ **8/8**; log-N **8/8**; ECM through **64-bit 8/8**; textbook factors **18/18** | They demo \(N=15\). We factor RSA-**shaped** moduli through 64-bit. **RSA-2048 not run** (smoothness / √p wall). |
...
1. **Shor’s end-job** — factor RSA-shaped far primes when p±1 are unsmooth. ECM through **64-bit**. RSA-2048 still the smoothness / √p wall (not a pretend factor).
