...
**Current record (2026-09-10):** competitor climb closed on this pin. Audit **20/20** vs YR4/PDG, leftover hired physics **41/41**. Three earlier audit misses were wrong objects (`docs/MISS_THREE.md`), not a pin edit. Exclusive \(B\to D\ell\nu\) is **0.15%** (`docs/V_CB_PUZZLE.md`) — the old 1.1σ was the D+D* blend 0.0398. SH0ES \(H_0\) is Lean BH→WH (Planck **0.024%**, SH0ES **1.00%**) — `docs/H0_TENSION.md`. Gset family **11/11 champions**. RSA-100 (330-bit) **miss** at locked B (`docs/RSA100.md`). Next climb RSA-129 (426-bit). RSA-shaped through **119-bit 8/8** (60-bit ρ); **64-bit 8/8** (CFRAC + Brent ρ + SIQS). Multiprover **FSOT_QUANTUM_MULTIPROVER_OK** (35 obligations). Known-answer QC **44/44**. Contested Lean panel **14/14**. GitHub About text is the repo one-liner; this README, `docs/STATUS.md`, and `docs/CLAIMS.md` are the current record.
...
| Heights 16 (127-bit RSA-shaped) | **8/8** CFRAC 5 + Brent ρ 3 | [`docs/HEIGHTS16.md`](docs/HEIGHTS16.md) |
...
**Still open, written as open:** RSA-100 (330-bit) **miss** at locked B; next named climb is RSA-129. RSA-2048 not run. Vendor `BR_H_gg` field still stale (fold already matches YR4). Gset unweighted **11/11 champions**. Exclusive 0.0398 and SH0ES 6.30% were wrong objects — see [`docs/CLAIMS.md`](docs/CLAIMS.md).
...
python -m fsot_quantum heights16  # 64-bit RSA-shaped 8/8 CFRAC + Brent ρ
