...
**Still open, written as open:** Gset champions unmatched (G17 13 edges, G22 98); RSA-shaped **64-bit 0/8** at locked ECM (ρ \(\sim 2^{32}\) wall); vendor `BR_H_gg` field still stale (fold already matches YR4). RSA-2048 not run. Exclusive 0.0398 and SH0ES 6.30% were wrong objects — see [`docs/CLAIMS.md`](docs/CLAIMS.md).
