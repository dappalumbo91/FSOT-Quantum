"""
Hired QC climb 6 — 11-digit factor and harder QAOA/HHL jobs.

After hire5 (10-digit through 1445900429). Same fold law.
No foreign circuit. No new coefficient. Not genetics.

python -m fsot_quantum.hire_climb6
python -m fsot_quantum hire6
"""
...
FACTOR_N: tuple[int, ...] = (
    ...
    10032225857,  # 100153 × 100169
    10040039951,  # 100193 × 100207
    10045050481,  # 100213 × 100237
    10015400453,  # 100003 × 100151  farther
    10025008901,  # 100043 × 100207
)

DLOG: tuple[tuple[int, int, int, int], ...] = (
    _dlog_row(3, 610, 2000003),
    _dlog_row(5, 987, 2500009),
    _dlog_row(6, 1597, 3000017),
    _dlog_row(7, 2584, 4000037),
)
...
def _sat28() -> tuple[int, tuple[tuple[tuple[int, int], ...], ...], list[int]]:
    n = 28
    ...
    for k in range(56):
        idx = [((phi * (k + 7) + i * 29) % n) for i in range(3)]
        if len(set(idx)) < 3:
            idx = [(k + 5 * i) % n for i in range(3)]
        clauses.append(tuple((i, bool(wit[i])) for i in idx))
    return n, tuple(clauses), wit


def _lin7() -> tuple[list[list[int]], list[int], list[int], int]:
    n = 7
...
def main() -> int:
    ...
    tsp = fold_tsp(_tsp_metric(10))
    rows.append({
        "family": "tsp",
        "question": "TSP n=10 on the seed metric — match exact tour length?",
        "hire": "QAOA / annealer",
    ...
    A, b, x_true, box = _lin7()
    lin = fold_linear_system(A, b, box)
    rows.append({
        "family": "linear",
        "question": "Solve 7×7 Ax=b?",
        "hire": "HHL",
        "answer": lin.get("x"),
    ...
    c11 = tuple((i, (i + 1) % 11) for i in range(11))
    mis = fold_independent_set(11, c11)
    rows.append({
        "family": "mis",
        "question": "Max independent set of the 11-cycle?",
        "hire": "QAOA",
        "answer": {"size": mis.get("size"), "exact": mis.get("exact")},
        "ok": bool(mis.get("ok") and mis.get("exact") == 5),
        "method": mis.get("method"),
    })

    part = fold_partition(list(range(1, 36)))
    rows.append({
        "family": "partition",
        "question": "Partition {1..35} into two equal-sum sets?",
        "hire": "QAOA / QUBO",
    ...
    report = {
        ...
        "previous": "hire_climb5 22/22 through 1445900429",
        "wall_seconds": time.perf_counter() - t0,
        "rows": rows,
    }
    out = ROOT / "results"
    out.mkdir(exist_ok=True)
    (out / "hire_climb6.json").write_text(json.dumps(report, indent=2), encoding="utf-8")
    ...
    md = [
        ...
        "",
        "After `hire5`. Same modular / energy folds. Factors through "
        "**10,045,050,481**. Discrete log through **p = 4,000,037**. "
        "SAT-28, TSP n=10, 7×7 HHL, C11 independent set.",
        "",
        "| Family | Hire | Score |",
        "|--------|------|------:|",
    ]
    ...
    text = "\n".join(md)
    (out / "HIRE_CLIMB6.md").write_text(text, encoding="utf-8")
    (ROOT / "docs" / "HIRE_CLIMB6.md").write_text(text, encoding="utf-8")
    print(json.dumps({
        "overall_ok": ok,
        "score": f"{n_ok}/{n}",
