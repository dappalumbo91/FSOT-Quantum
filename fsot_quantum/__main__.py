...
Field entry: python -m fsot_quantum [cmd]
  ...
  heights13  RSA-shaped 48-bit × 48-bit (95-bit N)
  heights14  RSA-shaped 52-bit × 52-bit (~103-bit N)
...
def main() -> int:
    ...
    if c in ("heights3", "height3"):
        ...
        return m()
    if c in ("heights4", "height4"):
        from fsot_quantum.heights4 import main as m
        return m()
    ...
    if c in ("heights8", "height8"):
        ...
        return m()
    if c in ("heights9", "height9"):
        from fsot_quantum.heights9 import main as m
        return m()
    if c in ("heights10", "height10"):
        from fsot_quantum.heights10 import main as m
        return m()
    ...
    if c in ("heights12", "height12"):
        ...
        return m()
    if c in ("heights13", "height13"):
        from fsot_quantum.heights13 import main as m
        return m()
    if c in ("heights14", "height14"):
        from fsot_quantum.heights14 import main as m
        return m()
    if c in ("heights15", "height15"):
        from fsot_quantum.heights15 import main as m
        return m()
    if c in ("heights16", "height16"):
        from fsot_quantum.heights16 import main as m
        return m()
