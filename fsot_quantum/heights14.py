"""
Classical-record direction: RSA-shaped 52-bit × 52-bit (~103-bit N).

heights13 was 48-bit × 48-bit (95-bit). Same job cryptographers mean:
...
BAL52: tuple[tuple[int, int], ...] = (
    ...
    (2300000000000453, 3200000000000567),
    (2400000000000151, 3000000000000463),
    (2400000000000151, 3400000000000679),
    (2500000000000163, 3200000000000567),
    (2500000000000163, 3600000000000799),
    (2600000000000203, 3400000000000679),
    (2700000000000217, 3600000000000799),
)
...
def main() -> int:
    ...
    rows = [_row(p, q) for p, q in BAL52]
    n = len(rows)
    ...
    (out / "heights14.json").write_text(json.dumps(report, indent=2), encoding="utf-8")
    ...
    text = "\n".join(md)
    (out / "HEIGHTS14.md").write_text(text, encoding="utf-8")
    (ROOT / "docs" / "HEIGHTS14.md").write_text(text, encoding="utf-8")
    print(json.dumps({
        "overall_ok": ok,
        "score": f"{n_ok}/{n}",
