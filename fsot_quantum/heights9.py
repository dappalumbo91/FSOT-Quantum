"""
Next real move: ECM at ~56-bit far p±1-unsmooth.

heights8 was 52-bit. This board is ~4e7 × 1.5e9. Same B / B2,
...
def main() -> int:
    ...
    (out / "heights9.json").write_text(json.dumps(report, indent=2), encoding="utf-8")
    ...
    md = [
        ...
        "",
        "heights8 was 52-bit. This board is ~56-bit (`4e7 × 1.5e9`). "
    ...
    md += [
        "",
        f"RSA-2048: B=`{B}` still (not run). 56-bit is not 2048-bit.",
        "",
        "```powershell",
        "python -m fsot_quantum.heights9",
        "```",
    ...
    text = "\n".join(md)
    (out / "HEIGHTS9.md").write_text(text, encoding="utf-8")
    (ROOT / "docs" / "HEIGHTS9.md").write_text(text, encoding="utf-8")
    print(json.dumps({
        "overall_ok": ok,
        "ecm": f"{ecm_ok}/{n}",
        "logN": f"{log_ok}/{n}",
