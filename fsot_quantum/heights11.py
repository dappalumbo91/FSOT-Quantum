"""
Next real move: ECM at 80-bit far p±1-unsmooth.

heights10 was 64-bit. This board is ~4e9 × 3e14. Same B / B2,
...
ECM11_N: tuple[tuple[int, int], ...] = (
    ...
    (4000000019, 300000000000097),
    (4000000019, 300000000000179),
    (4000000019, 300000000000187),
    (4000000063, 300000000000089),
    (4000000063, 300000000000097),
    (4000000133, 300000000000089),
    (4000000187, 300000000000089),
)
...
def main() -> int:
    ...
    (out / "heights11.json").write_text(json.dumps(report, indent=2), encoding="utf-8")
    ...
    md = [
        ...
        "",
        "heights10 was 64-bit. This board is **80-bit** (`4e9 × 3e14`). "
    ...
    md += [
        "",
        f"RSA-2048: B=`{B}` still (not run). 80-bit is not 2048-bit.",
        "",
        "Next wall (same B / B2, not retuned): a 90-bit far pair "
        "`8000000081 × 100000000000000003` returns `ecm_exhausted` "
        "(58344 steps). The 8-digit-scale factor is no longer B-smooth "
        "in the elliptic group. Written, not crawled.",
        "",
        "```powershell",
        "python -m fsot_quantum.heights11",
        "```",
    ...
    text = "\n".join(md)
    (out / "HEIGHTS11.md").write_text(text, encoding="utf-8")
    (ROOT / "docs" / "HEIGHTS11.md").write_text(text, encoding="utf-8")
    print(json.dumps({
        "overall_ok": ok,
        "ecm": f"{ecm_ok}/{n}",
        "logN": f"{log_ok}/{n}",
