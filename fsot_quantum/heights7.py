"""
Next real move: ECM at ~48-bit far p±1-unsmooth.

heights6 was 46-bit. This board is ~3e6 × 8e7. Same B / B2,
...
def main() -> int:
    ...
    (out / "heights7.json").write_text(json.dumps(report, indent=2), encoding="utf-8")
    ...
    md = [
        ...
        "",
        "heights6 was 46-bit. This board is ~48-bit (`3e6 × 8e7`). "
    ...
    md += [
        "",
        f"RSA-2048: B=`{B}` still (not run). 48-bit is not 2048-bit.",
        "",
        "```powershell",
        "python -m fsot_quantum.heights7",
        "```",
    ...
    text = "\n".join(md)
    (out / "HEIGHTS7.md").write_text(text, encoding="utf-8")
    (ROOT / "docs" / "HEIGHTS7.md").write_text(text, encoding="utf-8")
    print(json.dumps({
        "overall_ok": ok,
        "ecm": f"{ecm_ok}/{n}",
        "logN": f"{log_ok}/{n}",
