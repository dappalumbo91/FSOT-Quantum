"""
Classical-record direction: RSA-shaped 56-bit × 56-bit (~111-bit N).

heights14 was 52-bit × 52-bit (103-bit). Same job cryptographers mean:
two similar-bit primes, not twins, p±1 unsmooth at our B. End-job
ECM or Pollard ρ. Consumer PC. Not RSA-100 / RSA-2048.
...
BAL56: tuple[tuple[int, int], ...] = (
    (40000000000000063, 50000000000000051),
    (40000000000000063, 52000000000000123),
    (42000000000000109, 50000000000000051),
    (42000000000000109, 55000000000000051),
    (44000000000000041, 52000000000000123),
    (44000000000000041, 58000000000000033),
    (46000000000000103, 55000000000000051),
    (46000000000000103, 58000000000000033),
)
...
def main() -> int:
    ...
    rows = [_row(p, q) for p, q in BAL56]
    n = len(rows)
    ...
    (out / "heights15.json").write_text(json.dumps(report, indent=2), encoding="utf-8")
    ...
    text = "\n".join(md)
    (out / "HEIGHTS15.md").write_text(text, encoding="utf-8")
    (ROOT / "docs" / "HEIGHTS15.md").write_text(text, encoding="utf-8")
    print(json.dumps({
        "overall_ok": ok,
        "score": f"{n_ok}/{n}",
